...
-- Add new check constraint with 'welcome' type
ALTER TABLE custom_code 
ADD CONSTRAINT custom_code_type_check 
CHECK (type IN ('html', 'css', 'js', 'chat', 'welcome'));

-- Create policy for public access to welcome screen
CREATE POLICY "Anyone can view welcome screen"
...
CREATE INDEX IF NOT EXISTS idx_custom_code_welcome
ON custom_code(type, is_active)
WHERE type = 'welcome';

-- Insert default welcome screen if not exists
DO $$
BEGIN
  IF NOT EXISTS (
